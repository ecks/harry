...
struct list * get_sisis_addrs_for_prefix(struct prefix_ipv6 * p)
{
	...
	struct list * rib = calloc(1, sizeof(*rib));
        list_init(rib);
	...
	struct list * rtn = calloc(1, sizeof(*rtn));
        list_init(rtn);
	if (rib != NULL && rtn != NULL)
	{
		struct route_ipv6 * route;
		LIST_FOR_EACH(route, struct route_ipv6, node, rib)
		{
			int match = 1;
	...
	//	FREE_LINKED_LIST(rib); TODO: need to free list
	}
...
int get_process_type_version_count(uint64_t process_type, uint64_t process_version)
{
        ...
        {   
          ...
          // Free memory
//          FREE_LINKED_LIST(addrs); TODO: figure out how delete and free lists
        }   
